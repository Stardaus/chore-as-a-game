...
export function AssignChoreModal({ chore, isOpen, onClose }: AssignChoreModalProps) {
  ...
  const [pendingChanges, setPendingChanges] = useState<
    Record<string, 'assign' | 'unassign' | null>
  >({});
  ...
  const getActiveAssignment = (childId: string) => {
    ...
    return assignments.find((a) => a.choreId === chore.id && a.childId === childId && !a.completed);
  };
        ...
        <div className="grid grid-cols-2 gap-3">
          {profiles.map((profile) => {
            ...
            const willBeAssigned =
              (isCurrentlyAssigned && change !== 'unassign') ||
              (!isCurrentlyAssigned && change === 'assign');
            ...
            return (
              ...
              >
                ...
                {change === 'assign' && (
                  <span className="text-[10px] text-indigo-600 font-bold mt-1 animate-pulse">
                    + Assigning
                  </span>
                )}
                {change === 'unassign' && (
