...
export function ChoreBank() {
  ...
  const handleArchive = (id: string, title: string) => {
    ...
    if (isAssigned) {
      if (
        !confirm(
          `"${title}" is currently assigned. Archiving it will remove it from quests. Are you sure?`
        )
      ) {
        return;
      }
    ...
    <div className="space-y-6 pb-12">
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <span className="text-[10px] text-slate-500 font-medium text-center leading-tight mt-1 opacity-70">
            Assign routines by #tag to multiple kids
          </span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] text-slate-500 font-medium text-center leading-tight mt-1 opacity-70">
            Add daily prayers & common tasks instantly
          </span>
        </button>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleAddChore} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">
                  Tags (comma separated, max 50 chars)
                </label>
                <Input
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                disabled={!canAddChore}
              />
              <label htmlFor="approval" className="text-sm font-medium text-slate-700 select-none">
            ...
            <Button
              ...
              disabled={!canAddChore || !title.trim()}
            >
          ...
          {!canAddChore && (
            <p className="text-xs text-amber-600 mt-2 font-medium">
              Free plan limit reached ({FREE_TIER_LIMITS.CHORES} Chores). Upgrade to Premium to add
              more.
            </p>
          )}
        </CardContent>
        ...
        <div className="grid gap-4">
          {activeChores.map((chore) => {
            const currentAssignments = assignments.filter(
              (a) => a.choreId === chore.id && isCurrentAssignment(a, chore)
            );
            const isAssigned = currentAssignments.length > 0;
            const assignedProfiles = profiles.filter((p) =>
              currentAssignments.some((a) => a.childId === p.id)
            );
              ...
              <Card
                ...
                className={cn(
                  ...
                  isAssigned
                    ...
                    : 'border-slate-100 opacity-80 shadow-none'
                )}
              >
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          {isAssigned ? (
                            ...
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-bold rounded-full uppercase tracking-wider italic">
                              Inactive
                            </span>
                          )}
                  ...
                  {chore.tags && chore.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {chore.tags.map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-0.5 bg-white text-slate-500 rounded-md text-[10px] font-bold border border-slate-200"
                        >
                          #{tag}
                        </span>
                      ))}
                        ...
                        <>
                          <div className="flex -space-x-2 shrink-0">
                            {assignedProfiles.map((p) => (
                              <div
                                key={p.id}
                                className="h-7 w-7 rounded-full border-2 border-white bg-slate-100 overflow-hidden shadow-sm"
                                title={p.name}
                              >
                                <img
                                  src={p.avatar}
