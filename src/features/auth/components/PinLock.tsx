...
export function PinLock({ onUnlock }: PinLockProps) {
    ...
    const generateMathProblem = () => {
        const num1 = Math.floor(Math.random() * 12) + 7; // 7-18
        ...
        setMathProblem({ question: q, answer: a });
        setAnswer('');
        setError(null);
    };
    ...
    const handleRecoverySubmit = (e: React.FormEvent) => {
        ...
        const isCorrect = recoveryMode === 'question' 
            ? answer.toLowerCase().trim() === recoveryAnswer.toLowerCase().trim()
            : parseInt(answer) === mathProblem.answer;
