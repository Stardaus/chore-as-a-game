...
export const createChoreSlice = (set: StoreSet, get: StoreGet): ChoreSlice => ({
  ...
  addChore: async (chore) => {
    ...
    if (active.some((c) => c.title.toLowerCase() === result.data!.title.toLowerCase())) {
      alert('Chore exists!');
      return;
    }
    ...
    if (familyId) {
      await safeSync('chores', 'insert', {
        ...
        requires_approval: newChore.requiresApproval,
        icon: newChore.icon,
        tags: newChore.tags,
  ...
  deleteChore: async (id) => {
    ...
    set({
      ...
      assignments: assignments.filter((a) => a.choreId !== id),
    });
  ...
  archiveChore: async (id) => {
    ...
    if (familyId)
      await safeSync('chores', 'update', { status: 'archived' }, { column: 'id', value: id });
  },
  ...
  assignChore: async (choreId, childId) => {
    ...
    if (familyId)
      await safeSync('assignments', 'insert', {
        id: newAssignment.id,
        family_id: familyId,
        profile_id: childId,
        chore_id: choreId,
  ...
  unassignChore: async (choreId, childId) => {
    ...
    set({
      assignments: assignments.filter(
        (a) => !(a.choreId === choreId && a.childId === childId && !a.completed)
      ),
    });
    if (familyId) {
      try {
        ...
        console.error('Offline delete unsupported for unassign');
      }
    }
  },
  ...
  toggleAssignment: async (id) => {
    ...
    set({
      assignments: assignments.map((x) =>
        x.id === id
          ? { ...x, completed: isCompleting, completedAt: isCompleting ? now : undefined }
          : x
      ),
    });
    if (familyId) {
    ...
    const nextProfiles = profiles.map((p) =>
      p.id === a.childId
        ? {
            ...
            xp: p.xp + c.points,
            level: Math.floor((p.xp + c.points) / 100) + 1,
          }
        : p
    );

    set({
      assignments: assignments.map((x) => (x.id === id ? { ...x, verifiedAt } : x)),
    ...
    if (familyId) {
      ...
      if (p)
        await safeSync(
          'profiles',
          'update',
          { points: p.points, xp: p.xp, level: p.level },
          { column: 'id', value: p.id }
        );
    }
  },
  ...
  refreshAssignments: () => {
    ...
    profiles.forEach((profile) => {
      chores
        .filter((c) => c.status === 'active' && c.frequency !== 'one-time')
        .forEach((chore) => {
          const active = assignments.some(
            (a) => a.choreId === chore.id && a.childId === profile.id && !a.completed
          );
          if (!active) {
  ...
  assignChoresByTag: async (tag, childIds) => {
    ...
    childIds.forEach((childId) => {
      toAssign.forEach((chore) => {
        if (
          !assignments.some((a) => a.choreId === chore.id && a.childId === childId && !a.completed)
        ) {
          newAssignments.push({
            id: uuidv4(),
            choreId: chore.id,
