...
export const SyncEngine = {
  ...
  /**
   * Queue an operation to syncQueue state and SW IndexedDB queue.
   */
  queueOperation: (op: {
    table: string;
    action: 'insert' | 'update' | 'delete';
    ...
    state.queueSyncOperation(fullOp);

    // Also write to Service Worker accessible IndexedDB queue
    import('../lib/sw-sync-queue').then(({ SWSyncQueue }) => {
      SWSyncQueue.addOperation(fullOp);
    });

    // Register Service Worker Background Sync event if supported
    if ('serviceWorker' in navigator && 'SyncManager' in window) {
      navigator.serviceWorker.ready
        .then((reg) => {
          return (reg as any).sync.register('sync-chorequest-queue');
        })
        .catch((err) => {
