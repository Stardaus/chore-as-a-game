...
export const Validation = {
  ...
  chore: (data: { title: string; points: number; tags?: string[] }) => {
    ...
    if (!title || title.length > 40)
      return { valid: false, error: 'Title must be 1-40 characters.' };

    if (data.points < 1 || data.points > 10000)
      return { valid: false, error: 'Points must be between 1 and 10,000.' };

    if (data.tags && data.tags.some((t) => t.length > 20))
      return { valid: false, error: 'Individual tags must be under 20 chars.' };
  ...
  reward: (data: { title: string; cost: number }) => {
    const title = data.title.trim();
    if (!title || title.length > 40)
      return { valid: false, error: 'Reward title must be 1-40 characters.' };

    if (data.cost < 1 || data.cost > 50000)
      return { valid: false, error: 'Cost must be between 1 and 50,000.' };
  ...
  security: (data: { pin?: string; question?: string; answer?: string }) => {
    ...
    if (data.question !== undefined) {
      if (data.question.length > 100)
        return { valid: false, error: 'Question too long (max 100).' };
    }
  ...
  device: (data: { name: string }) => {
    ...
    if (!name || name.length > 20)
      return { valid: false, error: 'Device name must be 1-20 characters.' };
    return { valid: true, data: { ...data, name } };
  },
};
